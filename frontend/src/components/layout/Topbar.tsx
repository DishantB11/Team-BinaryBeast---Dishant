...
export const Topbar: React.FC = () => {
  ...
  return (
    <header className="fixed top-0 left-[260px] right-0 h-[60px] bg-[#1e1e1e] border-b border-[#3d3d3d] flex items-center justify-between px-6 z-10">
      ...
      {/* Notion Search Bar Mock */}
      <div className="relative w-64">
        <span className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
          <Search className="h-4 h-4 text-[#6b6b6b]" />
        </span>
        <input
          type="text"
          placeholder="Search planner..."
          className="w-full bg-[#2d2d2d] border border-[#3d3d3d] rounded-md py-1.5 pl-9 pr-4 text-sm text-white placeholder-[#6b6b6b] focus:outline-none focus:border-[#fbbf24] transition-colors"
          disabled
        />
      </div>
    </header>
